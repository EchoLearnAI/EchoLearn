locals {
  // Map full region names, CLI names and slugs to slugs
  region_slug = lookup(zipmap(values(local.regions), keys(local.regions)), var.name, lookup(zipmap(values(local.cli_names), keys(local.cli_names)), var.name, var.name))
